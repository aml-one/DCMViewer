...
public sealed class MainViewModel : INotifyPropertyChanged
{
    ...
    public double LightingStrength
    {
        get => _lightingStrength;
        set
        {
            var clampedValue = Math.Clamp(value, 0.4, 1.8);
            if (Math.Abs(_lightingStrength - clampedValue) < 0.0001)
            {
                return;
            }

            _lightingStrength = clampedValue;
            ApplyLightingStrength();
            OnPropertyChanged();
        }
    }
    ...
    public async Task LoadFilesAsync(IEnumerable<string> filePaths, bool clearExisting)
    {
        ...
        try
        {
            ...
            foreach (var filePath in normalizedPaths)
            {
                ...
                {
                    var failedFile = LoadedMeshItemViewModel.CreateFailed(filePath, ex.Message);
    ...
    {
        // 1. Explicit per-filename override from the host application.
        var filename = Path.GetFileName(filePath);
        if (TextureOverrides.TryGetValue(filename, out var overrideName) &&
    ...
    private void ReapplyDefaultTexture(string previousTexture, string newTexture)
    {
        ...
        foreach (var loadedFile in _loadedFiles)
        {
            var filename = Path.GetFileName(loadedFile.FilePath);
            if (!TextureOverrides.ContainsKey(filename) &&
    ...
    {
        var fileName = Path.GetFileName(filePath);
        if (CategoryOverrides.TryGetValue(fileName, out var overrideValue) &&
